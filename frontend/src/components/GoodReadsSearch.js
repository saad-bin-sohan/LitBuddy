...
const GoodReadsSearch = ({ onImportBook, onClose }) => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      setLoading(true);
      setError(null);
      const results = await goodreadsApi.searchBooks(searchQuery.trim(), 1);
  ...
  const handlePageChange = async (page) => {
    try {
      setLoading(true);
      const results = await goodreadsApi.searchBooks(searchQuery.trim(), page);
      setSearchResults(results.results || []);
      setCurrentPage(page);
    } catch (err) {
      setError(err.message);
  ...
  const handleBookSelect = async (book) => {
    try {
      setLoading(true);
      const detailedBook = await goodreadsApi.getBookById(book.goodreadsId);
  ...
  return (
    <div className="goodreads-search-overlay">
      <div className="goodreads-search-modal">
        <div className="modal-header">
          <h2>Search GoodReads</h2>
            ...
            <div className="results-grid">
              {searchResults.map((book) => (
                <div 
                  ...
                  className={`result-item ${selectedBook?.goodreadsId === book.goodreadsId ? 'selected' : ''}`}
